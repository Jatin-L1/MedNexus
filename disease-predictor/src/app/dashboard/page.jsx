...
const DoctorDashboard = () => {
  ...
  // Initialize user state without directly accessing localStorage
  ...
  // Safely load user from localStorage (client-side only)
  useEffect(() => {
    const savedUser = localStorage.getItem("user");
    if (savedUser) {
      setUser(JSON.parse(savedUser));
  ...
  /** ✅ WebSocket Connection - Client side only */
  useEffect(() => {
      ...
      <div className="pt-16 flex h-screen">
        ...
        <div className="flex-1 bg-gray-900 p-6">
          <div className="bg-black/30 backdrop-blur-md border border-gray-800 rounded-lg h-full p-4">
            ...
            <div className="bg-gray-800/50 rounded-lg h-[calc(100%-4rem)] flex items-center justify-center">
              <div className="flex-1 p-6 overflow-y-auto">
                {responderLocation && blueIconRef.current && redIconRef.current && (
                  <MapContainer
                    key={`${responderLocation.lat}-${responderLocation.lng}`}
