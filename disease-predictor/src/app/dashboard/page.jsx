...
const toggleResponderStatus = async () => {
      ...
      navigator.geolocation.watchPosition(
        (position) => {
          ...
          if (socket && socket.readyState === WebSocket.OPEN) {
            ...
            socket.send(JSON.stringify({ responderId: user.id, lat: latitude, lng: longitude }));
          }
          ...
          if (emergencyRequests.length > 0) {
            ...
            acceptedEmergencies.forEach((emergency) => {
              ...
              .then(response => {
                console.log(`✅ Location updated for accepted emergency ${emergency._id}:`, response.data);
              })
              .catch(error => {
                console.error(`❌ Error updating location for accepted emergency ${emergency._id}:`, error.response ? error.response.data : error.message);
              });
            });
          }
        },
  ...
  useEffect(() => {
    if (user?.role === "doctor" || user?.role === "ambulance" || user?.role === "nurse") {
            ...
            <div className="space-y-2 text-gray-300">
              ...
              <div className="flex space-x-4 mt-4">
                <div className="bg-gray-800/50 rounded-lg p-3 flex-1 text-center">
                  ...
                  <p className="text-xl font-bold">{emergencyRequests.length}</p>
                </div>
  ...
  {emergencyRequests.map((emergency) => (
    ...
    >
      <div className="flex justify-between items-start mb-2">
        <h3 className="font-semibold">{emergency.patientId?.name}</h3>
        <span
      ...
      <div className="flex space-x-2">
        ...
        <div className="flex-1 bg-gray-900 p-6">
          <div className="bg-black/30 backdrop-blur-md border border-gray-800 rounded-lg h-full p-4">
            ...
            <MapContainer 
  key={responderLocation ? `${responderLocation.lat}-${responderLocation.lng}` : 'default'} 
  center={[responderLocation?.lat || 0, responderLocation?.lng || 0]} 
  zoom={20} 
...
>
          ...
          {emergencyRequests.map((emergency) => (
            <Marker 
              key={emergency._id} 
              position={[emergency.patientLocation.lat, emergency.patientLocation.lng]} 
              icon={blueIcon}
            >
              <Popup>{emergency.patientId?.name || "Unknown Patient"}</Popup>
            </Marker>
          ))}
